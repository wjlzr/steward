...
use App\Models\Eoa\Release;
use App\Models\Eoa\ReleaseFiles;
...
class ReleaseController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        return response()->json(['code'=>200, 'message'=>'ok', 'data'=>$res_data]);
    ...
    public function getRevisions(Request $request)
    {
        ...
        $user_id = Redis::get('ST_EOA_USER_ID_' . $token);
        if (!$user_id) {
            return response()->json(['code'=>100000, 'message'=>'请先登录']);
        }
        ...
        foreach($log_obj->logentry as $logentry) {
            ...
            if ($logentry->author == $svn_id) { //属于当前用户的记录
                ...
                $release = Release::select('release_id')
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getReleasedFiles($env, $revision_number)
    {

        $release_file = ReleaseFiles::where('version_number', $revision_number)->get();
        if ($release_file) {
            $files = [];
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function release(Request $request, $revision_number)
    {
