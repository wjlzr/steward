...
class Curl
{
    ...
    public function get_access_token($code)
    {
        ...
        error_log('=====get token');
        error_log(var_export($res_token,1));

        if ($res_token['code'] != 200) {
            return ['code'=>400, 'message'=>$res_token['message']];
        }

        if (!isset($res_token['data']['access_token'])) {
            return ['code'=>400, 'message'=>'access_token获取失败！'];
        }
    ...
    public function refresh_access_token()
    {
        ...
        if (!isset($res_token['data']['access_token'])) {
            return ['code'=>400, 'message'=>'access_token刷新失败！'];
        }
