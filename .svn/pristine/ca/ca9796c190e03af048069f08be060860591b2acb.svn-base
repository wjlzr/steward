            ...
            foreach ( $st_app_goods_sale['data'] as $item){
                ...
                if( in_array( $item['spec_id'] , $spec_ids )){
                    ...
                    foreach ( $goods_app[ $item['spec_id']]['app_id'] as $app){
                        ...
                        $plat_form .= '<img style="padding:2px;" src="'.$app_map[ $app ]['logo'] .'">';
                    }
                    ...
                    $result[ $item['spec_id']] = [
                        ...
                        'category' => $category ,
                        'price' => '<span>'.$price. '</span><a href="#" class="price" data-id="'. $item['spec_id'] .'" data-name="'.$item['name'].'"><img src="/images/admin/updates.png" width="30px;" style="margin-top:-3px;"></a>' ,
                        'enable_number' => '<span>'.$enable_number . '</span><a href="#" class="inventory" data-id="'. $item['sku'] .'" data-name="'.$item['name'].'&nbsp;&nbsp;'.$item['spec'].'"><img src="/images/admin/updates.png" width="30px;" style="margin-top:-3px;"></a>',
                        'platForm' => $plat_form
                    ];
                    ...
                    $price = $item['price'];
                    $plat_form = '<img style="padding:2px;" src="'.$app_map[ $item['app_id'] ]['logo'] .'">';
                    $enable_number = $item['enable_number'] == NULL ? 0 : $item['enable_number'];
        ...
        if( !empty($app_ids)){
            ...
            foreach ( $st_app_goods_sale as  $item ){
                ...
                if( $form_value[0]['enable_number'] != $st_goods_stock -> enable_number ){
                    ...
                    $app_enable_number = $form_value[0]['enable_number'] - $st_goods_stock -> lock_number < 0
                        ? 0 : $form_value[0]['enable_number'] - $st_goods_stock -> lock_number ;
                    //应用平台同步库存
