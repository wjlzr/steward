...
class EleMeGoodsRequest implements GoodsFactoryInterface
{
    ...
    public function create_category($args_data)
    {
        ...
        if ($st_category->p_id == 0 && $st_category->level == 1) {
            ...
            if (!$st_app_category->first()) {
                return $this->curl->response('父级分类信息未找到');
            }
    ...
    public function update_category($args_data)
    {
        ...
        $st_app_category = StAppCategory::where(['category_id' => $args_data['category_id'], 'mall_id' => $args_data['mall_id'], 'app_id' => 100002])->first();

        $parent_id = 0;
        if ($st_app_category->level > 1) {
            $st_p_category = StAppCategory::where(['p_id' => $args_data['category_id'], 'mall_id' => $args_data['mall_id'], 'app_id' => 100002])->first();
            $parent_id = $st_p_category->o_category_id;
        }

        $res = $this->curl->call('eleme.product.category.updateCategory', [
        ...
        StAppCategory::where(['category_id' => $args_data['category_id'],
            'mall_id' => $args_data['mall_id'], 'app_id' => 100002])
    ...
    public function batch_update_self($args_data)
    {
        ...
        foreach ($args_data['goods'] as $goods_id) {
            $st_goods = StAppGoodsSale::select('o_goods_id')->where(['goods_id' => $goods_id, 'app_id' => 100002])->first();
            if (!$st_goods) {
        ...
        try {
            ...
            $status = 1;
            if ($args_data['is_shelf'] == 0) {
                $status = 2;
            }

            StAppGoodsSale::whereIn('spec_id', $args_data['goods'])->update(['status' =>$status]);
    ...
    public function batch_update_price($args_data)
    {
        ...
        foreach ($args_data['goods'] as $goods_specs) {
            foreach ($goods_specs as $spec_id => $price) {
                StAppGoodsSale::where('spec_id', $spec_id)->update(['price' => $price]);
            }
        }
        return $res;
    ...
    public function batch_delete_goods($args_data)
    {
        ...
        $res = $this->curl->call('eleme.product.item.batchRemoveItems', ['itemIds' => $itemIds]);
        ...
        foreach ($args_data as $goods) {
            StAppGoodsSale::whereIn('goods_id', $goods['goods'])->delete();
        }
    ...
    public function create_product($args_data)
    {
        ...
        $res = $this->upload_image($host . $args_data['goods_image']);
    ...
    public function update_product($args_data)
    {
        ...
        $res = $this->upload_image($host . $st_goods->image);
        ...
        foreach ($args_data['spec'] as $goods_specs) {
            ...
            $update['name'] = $args_data['goods_name'];
            $update['spec'] = $goods_specs['name'];
            $update['price'] = $goods_specs['price'];
            $update['sku'] = $goods_specs['product_code'];
            $update['upc'] = app_to_string($st_goods_sale->upc);
            $update['images'] = $host . $st_goods->image;
            StAppGoodsSale::where(['goods_id' => $args_data['goods_id'], 'spec_id' => $goods_specs['spec_id']])->update($update);
     ...
     * @return mixed
     */
    public function set_sort_category($args_data)
    {
        ...
        if (!isset($args_data['level']) || !in_array($args_data['level'], [1, 2, 3])) {
            return $this->curl->response('分类等级参数错误');
        }

        if (!isset($args_data['o_category_id']) || !is_array($args_data['o_category_id'])) {
            return $this->curl->response('缺少需要排序的分类列表');
        }
        ...
        $category_Ids = [];
        switch ($args_data['level']) {

            case 1:
                $res = $this->curl->call('eleme.product.category.setCategorySequence', ['shopId' =>
                    $st_app_mall->o_mall_id, 'categoryIds' => $args_data['o_category_id']]);
                break;

            case 2:
                $st_app_category = StAppCategory::where(['category_id' => $args_data['pid'], 'mall_id' => $args_data['mall_id'], 'app_id' => 100002]);
                $categoryWithChildrenIds[] = [
                    'parentId' => $st_app_category->o_category_id,
                    'childrenIds' => $args_data['o_category_id']
                ];
                $res = $this->curl->call('eleme.product.category.setCategoryPositionsWithChildren', [
                    'shopId' => $st_app_mall->o_mall_id,
                    'categoryWithChildrenIds' => $category_Ids
                ]);
                break;
