...
class businessController extends Controller
{
    ...
    public function search( Request $request )
    {
        ...
        $time = '前'. $days .'日    ';
        $next_start_date = date('Y-m-d',strtotime('-'. $days .' day',strtotime($startDate)));
        $next_end_date = date('Y-m-d',strtotime('-'. $days .' day',strtotime($endDate)));
        ...
        SUM(activity_expense) as activity_expense,SUM(service_expense) as service_expense,SUM(app_expense) as app_expense,SUM(total_cancel_mall_fee) as total_cancel_mall_fee'))
            ...
            $return = [
                ...
                'd3'=>$mall_data['total_sale_bill_num']>0 ? round( $mall_data['total_user_fee']/$mall_data['total_sale_bill_num'] ,2) : 0,
                ...
                'e3'=>isset( $mall_data['total_cancel_mall_fee'] ) ? round( $mall_data['total_cancel_mall_fee'],2) : 0,
            ];
