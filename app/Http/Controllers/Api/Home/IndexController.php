...
class IndexController extends Controller
{
    ...
    public function appOrdersSales(Request $request)
    {
        ...
        //平台
        $stApp = StApp::where('enable',1)->get();
    ...
    public function orderEfficiencyRank(Request $request)
    {
        ...
        if ($res_count->count() <= 0) {
            ...
            $return_result = [
                'low' => app_to_string(round(max($shopEffic_array))),
                'avg' => app_to_string(round(array_sum($shopEffic_array)/count($shopEffic_array))),
                'this' => app_to_string(round($shopEffic)),
                'high' => app_to_string(round(min($shopEffic_array))),
                'rank' => app_to_string($rank)
            ];
        }
