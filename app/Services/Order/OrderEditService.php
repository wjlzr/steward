...
use App\Services\Msg\Push as MsgPush;
...
class OrderEditService
{
    ...
    public function cancel($order_id, $reason_id = 0, $reason_content = '', $operator, $app_release = 1)
    {
        ...
        try {
            ...
            //库存释放
            $st_order_goods = StOrderGoods::where('order_id',$order_id)->get();
            $st_order = StOrder::find( $order_id );
            $sku_list = [];

            if( $pre_status == 4 ){

                if( !$st_order_goods -> isEmpty()){
                    foreach ( $st_order_goods as $goods ){

                        StGoodsStock::where([['mall_id',$st_order -> mall_id],[ 'sku',$goods -> goods_sku ]])->decrement('enable_number',$goods -> goods_number);
                        StGoodsStock::where([['mall_id',$st_order -> mall_id],[ 'sku',$goods -> goods_sku ]])->decrement('lock_number',$goods -> goods_number);
                        $sku_list[] = $goods -> goods_sku;
                    }
                }
            }else{
