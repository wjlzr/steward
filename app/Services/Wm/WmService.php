...
use App\Services\Queue\QueueService as Queue;
...
class WmService
{
    ...
    public function send( $api_name, $request = [] ) {
        ...
        if (!class_exists($class_name)) {
            $return = ['code'=>400, 'message'=>'接口无权访问，请检查接口文件是否存在！'];
            return $return;
        }
        ...
        $return_result = empty($request) ? $instance->$func_name() : $instance->$func_name($request);
    ...
    public function create_order( $order_data) {
        ...
        $push_result = Queue::async([
            'call_url' => 'http://' . $_SERVER['HTTP_HOST'] . '/ajax/order/add',
    ...
    public function push_message($user_array, $message, $type) {
        ...
        foreach ($user_array as $user) {
            ...
            $st_user_devices = StUserDevices::where('user_id', $user->id)->get();
            ...
            foreach($st_user_devices as $device) {
                ...
                $user_devices_push_first = StUserDevicesPush::where(['device'=>$device->device, 'type' => $type])
                                                    ->orderBy('id', 'desc')
                                                    ->first();

                if ($user_devices_push_first && (time() - strtotime($user_devices_push_first->updated_at)) <= 10) {
                    continue;
                }

                $user_devices_push = new StUserDevicesPush();
                $user_devices_push->creator = 'api-jpush';
                $user_devices_push->type = $type;
                $user_devices_push->device = $device->device;
                $user_devices_push->user_id = $device->user_id;
                $user_devices_push->content = $message;
                $user_devices_push->save();

                $push_device[] = $device->device;

            }

        }

        if (!empty($push_device)) {
     ...
     * @return array
     */
    public function async_repertory($args_data)
    {

        if (!isset($args_data['mall_id']) || !ebsig_is_int($args_data['mall_id'])) {
            return ['code'=>400, 'message'=>'缺少门店ID'];
        }

        if (!isset($args_data['sku_list']) || empty($args_data['sku_list'])) {
            return ['code'=>400, 'message'=>'没有需要同步的sku'];
        }
        ...
        foreach($app_array as $app) {
            $async_data = [
                'app_id' => $app->id,
                'mall_id' => $args_data['mall_id'],
                'sku_id' => $args_data['sku_list']
            ];
            $async_index = 'goods_async_' . $app->id . '_' . $args_data['order_id'];
            Redis::setex($async_index, 86400, json_encode($async_data));
            Queue::async([
                'call_url' => 'http://' . $_SERVER['HTTP_HOST'] . '/ajax/goods/push_app_async',
                'goods_async' => "$async_index"
            ]);
        }
